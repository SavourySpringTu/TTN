...
void xoaLop(DS_LOP &ds_lop,int i);
void suaLop(DS_LOP &ds_lop,char t_lop_edit[51],int x);
...
SINH_VIEN *kttrungSV_Lop(DS_LOP ds_lop,char m_sv[16],char m_lop[16]); 
...
void xoaLop(DS_LOP &ds_lop,int x)
{
	for(int i=x;i<=ds_lop.sl-1;i++)
	{
...
void suaLop(DS_LOP &ds_lop,char t_lop_edit[51],int x)
{
	strcpy(ds_lop.ds[x]->tenlop,t_lop_edit);
}
void giaiphongLop(DS_LOP &ds_lop)
{
	for (int i = 0; i < ds_lop.sl; i++)
...
SINH_VIEN *kttrungSV(DS_LOP ds_lop,char m_sv[16]) // TIM TREN TAT CA CAC LOP
{
	for(int i=0;i<ds_lop.sl;i++)
	{
...
SINH_VIEN *kttrungSV_Lop(DS_LOP ds_lop,char m_sv[16],char m_lop[16]) // TIM TREN 1 LOP
{
	int temp=kttrungLop(ds_lop,m_lop);
	for(SINH_VIEN *p=ds_lop.ds[temp]->first_SINH_VIEN->phead;p!=NULL;p=p->pnext)
	{
		if(strcmp(m_sv,p->masv)==0)
		{
			return p;
...
void xoaSV(DS_SINH_VIEN *&ds_sv,char m_sv[16])
{
	...
	else if(strcmp(ds_sv->ptail->masv,m_sv)==0)
	{
