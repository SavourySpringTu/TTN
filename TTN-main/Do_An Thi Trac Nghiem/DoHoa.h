...
void drawchitietBaiThi(PTR_MH p,DS_BAI_THI ds_bt,DS_LOP ds_lop,char m_lop[16],char m_sv[16],char m_mh[16],int trangct);

// xu ly
void sign();
void menu(DS_LOP &ds_lop,PTR_MH &p,DS_BAI_THI &ds_bt,int &slmh);
void SV(DS_LOP &ds_lop,PTR_MH &p,DS_BAI_THI &ds_bt,int &slmh);
void lop(DS_LOP &ds_lop,PTR_MH &p,DS_BAI_THI &ds_bt,int &slmh);
void monhoc(DS_LOP &ds_lop,PTR_MH &p,DS_BAI_THI &ds_bt,int &slmh);
void cauhoi(DS_LOP &ds_lop,PTR_MH &p,DS_BAI_THI &ds_bt,int &slmh);
void Thi(DS_LOP &ds_lop,PTR_MH &p,DS_BAI_THI &ds_bt,int slmh,char m_sv[16]);
void Diem(DS_LOP &ds_lop,PTR_MH &p,DS_BAI_THI &ds_bt,int &slmh);
...
void sign()
{
	...
	DS_LOP ds_lop;
	docfileLop(ds_lop); // LOP
	
	PTR_MH p=NULL;
	docfileMH(p,slmh); // MON HOC
	
	DS_BAI_THI ds_bt;	
	docfileBT(ds_bt); // BAI THI
	
	duyet_mh_doc_file_CH(p); // CAU HOI THI
	
	for(int i=0;i<ds_lop.sl;i++) // SINH VIEN & DIEM THI
	{
		ds_lop.ds[i]->first_SINH_VIEN=new DS_SINH_VIEN[1];
		...
		{
			z->first_DIEM_THI=new DS_DIEM_THI[1];
							...
							for(int i=0;i<ds_lop.sl;i++)
							{
								for(SINH_VIEN *x=ds_lop.ds[i]->first_SINH_VIEN->phead;x!=NULL;x=x->pnext)
								{
									if(strcmp(user,x->masv)==0 && strcmp(password,x->password)==0)
									{
										...
										PTR_MH p=NULL;
										docfileMH(p,slmh);	
										Thi(ds_lop,p,ds_bt,slmh,x->masv);
										break;
									}
								}
							} 
	...
	while(true)
	{
		if(ismouseclick(WM_LBUTTONDOWN))
		{
			...
			switch(mapID[x][y])
			{
				case 1:
					{
						SV(ds_lop,p,ds_bt,slmh);
						break;
					}
				case 2:
					{
						lop(ds_lop,p,ds_bt,slmh);
						break;
					}
				case 3:
					{
						monhoc(ds_lop,p,ds_bt,slmh);
						break;
					}
				case 4:
					{
						cauhoi(ds_lop,p,ds_bt,slmh);
						break;
					}
				case 5:
					{
						Diem(ds_lop,p,ds_bt,slmh);
						break;
					}
				case 6:
					{
						Thi(ds_lop,p,ds_bt,slmh,m_sv_sign);
						break;
					}					
				case 7:
					{
				...
				case 33: // XOA SINH VIEN
					{
						...
						SINH_VIEN *p=kttrungSV_Lop(ds_lop,m_sv,m_lop);
						if(p==NULL)
						{
							resetThongBao();
				...
				case 8: // SUA LOP
					{
						...
						{
							suaLop(ds_lop,t_lop_edit,temp_sua);
							drawDS_LOP(ds_lop,trang);
							resetThongBao();
							outtextxy(1030,150,"SUA THANH CONG !");
						}
...
void drawDS_DIEM(DS_LOP ds_lop,char m_lop[16],char m_mh[16])
{
	...
	TaoHCN(500,105,590,135,MAU_HONG,0,MAU_DEN);
	TaoHCN(595,105,825,135,MAU_TRANG,70,MAU_DEN); 
	GanID(595,105,825,135,70);
	setbkcolor(MAU_HONG);
	outtextxy(506,108,"MA SV");
	
	TaoHCN(500,150,550,180,MAU_XANH_LA,71,MAU_DEN);
	GanID(500,150,550,180,71);
	setbkcolor(MAU_XANH_LA);
	outtextxy(506,155,"XEM");
...
void drawchitietBaiThi(PTR_MH p,DS_BAI_THI ds_bt,DS_LOP ds_lop,char m_lop[16],char m_sv[16],char m_mh[16],int trangct)
{
	...
	TaoHCN(20,10,100,40,MAU_HONG,102,MAU_DEN); // THOAT 101
	GanID(20,10,100,40,102);
	setcolor(MAU_DEN);
	setbkcolor(MAU_HONG);
	settextstyle(10,0,2);
	...
	TaoHCN(35,760,115,790,MAU_HONG,105,MAU_DEN); // TRUOC 105
	GanID(35,760,115,790,105);
	TaoHCN(1200,760,1280,790,MAU_HONG,106,MAU_DEN); // SAU 106
	GanID(1200,760,1280,790,106);
	setbkcolor(MAU_HONG);
	outtextxy(40,765,"TRUOC");
	outtextxy(1205,765,"SAU");
	...
	create_circle(200,320,15,0);	
	create_circle(200,380,15,0);
	create_circle(200,440,15,0);
	create_circle(200,500,15,0);
	cout<<ds_bt.ds[0]->mamh<<endl;
	for(int i=0;i<ds_bt.sl;i++)
	{
		...
		if(strcmp(ds_bt.ds[i]->mamh,m_mh)==0 && strcmp(ds_bt.ds[i]->masv,m_sv)==0)
		{
			for(int j=0;j<ds_bt.ds[i]->slc;j++)
			{
				...
				for(int k=0;k<x->ds_ch.sl;k++)
				{	
					if(ds_bt.ds[i]->ds_ch_thi[trangct]==x->ds_ch.ds[k].ID)
					{
						if(ds_bt.ds[i]->ds_tl[trangct]=='A')
						{
							fillellipse(200,320,5,5);
						}
						else if(ds_bt.ds[i]->ds_tl[trangct]=='B')
						{
							fillellipse(200,380,5,5);
						}
						else if(ds_bt.ds[i]->ds_tl[trangct]=='C')
						{
							fillellipse(200,440,5,5);
						}
						else if(ds_bt.ds[i]->ds_tl[trangct]=='D')
						{
							fillellipse(200,500,5,5);
						}
						...
						setcolor(MAU_DEN);
						setbkcolor(MAU_TRANG);
						outtextxy(255,170,x->ds_ch.ds[k].noidung);
						outtextxy(255,310,x->ds_ch.ds[k].dapanA);
						outtextxy(255,370,x->ds_ch.ds[k].dapanB);
						outtextxy(255,430,x->ds_ch.ds[k].dapanC);
						outtextxy(255,490,x->ds_ch.ds[k].dapanD);
						return;
					}					
				}
			} 
	...
	while(true)
	{
		if(ismouseclick(WM_LBUTTONDOWN))
		{
			...
			switch(mapID[x][y])
			{
				...
				case 70:
					{
						nhapText(600,110,825,135,m_sv,16,0);
						break;
					}
				case 71:
					{
						...
						{
							resetThongBao();
							outtextxy(1030,150,"KHONG CO BAI THI !");							
						}
						else
						{
							drawchitietBaiThi(p,ds_bt,ds_lop,m_lop,m_sv,m_mh,trangct);
						}
						break;
					}						
				...
				case 105: // TRANG TRUOC CAU HOI KHI THI
					{
						if(trangct==0)
						{
							setbkcolor(MAU_XANH_DUONG_NHAT);
							outtextxy(550,750,"DA LA TRANG DAU !");						
						}
						else
						{
							trangct--;
				...
				case 106: // TRANG SAU CAU HOI KHI THI
					{
						int temp;
						for(int i=0;i<ds_bt.sl;i++)
						{
						...
						if(trangct==ds_bt.ds[temp]->slc-1)
						{
							setbkcolor(MAU_XANH_DUONG_NHAT);
							outtextxy(550,750,"DA LA TRANG cuoi !");
						}
						else
						{
							trangct++;
